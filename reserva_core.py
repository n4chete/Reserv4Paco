...
def hacer_reserva(data: dict, session: requests.Session) -> str:
    try:
        ...
        print(f"[DEBUG] Usuario: {usuario}, Fecha: {fecha}, Hora: {hora}, Pista: {pista_raw}, Deporte: {deporte}", flush=True)

        # Configuración por deporte
        if deporte == "tenis":
            pista_map = {str(i): f"{i:02}" for i in range(1, 7)}
            endpoint = "01"
            ...
            pista_map = {str(i): str(19 + i) for i in range(1, 8)}
            endpoint = "02"
            pista = pista_map.get(pista_raw, pista_raw)
        ...
        if not reserva_response.ok:
            return "❌ Error al cargar la página de reserva"
        ...
        try:
            ...
            print(f"[DEBUG] HTML de confirmación guardado en: {debug_path}", flush=True)
        except Exception as e:
            print(f"[ERROR] No se pudo guardar HTML: {e}", flush=True)
        ...
        print(f"[ERROR] Excepción en hacer_reserva: {str(e)}", flush=True)
